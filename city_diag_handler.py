...
class CityDiagHandler(DiagHandler):
    ...
    def __check_payment(self):
        ...
        if self._record_data["payment"] in Const.FAST_ETHERNET:
            self.__gigabit = False
        # 1 Gbit if payment is known or more than limit
        elif self._record_data["payment"] in Const.GIGABIT_ETHERNET:
            self.__gigabit = True
    ...
    def __check_switch_ip(self):
        return self.__check_local_ip(self._record_data["switch"]) or IPv4Address(self._record_data["switch"]) in Const.SWITCH_OTHER_LOCAL_SUBNET
    ...
    def _result_user_card(self):
        ...
        if any(value == 0 for key, value in self._correctly_filled.items() if key != "public_ip"):
            print("Не заполнены поля:", ", ".join(name for key, name in Const.KEY_OUTPUT.items() if key != "public_ip" and self._correctly_filled[key] == 0))
            all_correct = False
        
        # print obviously incorrect fields
        if any(value == -1 for key, value in self._correctly_filled.items()):
            print("Неверно заполнены поля:", ", ".join(name for key, name in Const.KEY_OUTPUT.items() if self._correctly_filled[key] == -1))
            all_correct = False
    ...
    def _find_actual_gateway(self):
        ...
        if self._record_data["street"] == Const.LENSOVETA_ADDRESS_GATEWAY["street"] and self._record_data["house"] == Const.LENSOVETA_ADDRESS_GATEWAY["house"]:
            self._gateway_manager = L3Manager(Const.LENSOVETA_ADDRESS_GATEWAY["gateway"], self._record_data["ip"])
            return
    ...
    def _check_vlan_subnet(self):
        ...
        super()._user_subnet_matches_ip_interface((self.__untagged_vlan_id, self.__switch_vlans[self.__untagged_vlan_id]), self._record_data["gateway"],
                                                   self.__mask_length, self._record_data["gateway"][-7:] if self.__direct_public_ip else None)
